...
`tools/list` at `https://gateway.pipeworx.io/ecosystems/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/ecosystems_get_package \
  -H 'Content-Type: application/json' \
  -d '{"name":"lodash"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/ecosystems_get_package`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "ecosystems": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-ecosystems"]
    }
  }
}
```
